    ...
    with open( CSV_FILE_PATH, 'r') as csvfile:
        ...
        for row in csvreader:
            if is_header:
                is_header = False
                continue
            insert_query = f"INSERT INTO telemetry (id,order_number,total,discount,buyer_id) VALUES ({row[0]}, {row[1]}, {row[2]},{row[3]},{row[4]});"
            insert_queries.append(insert_query)

        # Сохраняем запросы
        with open('./dag/dags/sql/insert_queries.sql', 'w') as f:
            for query in insert_queries:
                f.write(f"{query}\n")

# Определяем DAG
with DAG('postgres_to_postgres_dag',
    ...
    create_table = PostgresOperator(
        ...
        postgres_conn_id='my_conn',  # Название подключения
        sql="""
    ...
    run_insert_queries = PostgresOperator(
        ...
        postgres_conn_id='my_conn',  # Название подключения к PostgreSQL в Airflow UI
